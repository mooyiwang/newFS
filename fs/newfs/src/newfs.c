...
int newfs_mkdir(const char* path, mode_t mode) {
	/* TODO: 解析路径，创建目录 */
	struct newfs_dentry *parent_de;
	struct newfs_dentry *dentry;
	struct newfs_inode *inode;
	char name[MAX_NAME_LEN]; 
	char pa[MAX_NAME_LEN];
	strcpy(pa, path);
	parent_de = look_dentry(pa, name, 1);

	dentry = new_dentry(name, T_DIR);
	inode = newfs_alloc_inode(dentry);

	add_son(parent_de, dentry);
...
int newfs_getattr(const char* path, struct stat * newfs_stat) {
	...
	strcpy(pa, path);
	char name[MAX_NAME_LEN];
	dentry = look_dentry(pa, name, 0);
	if(dentry == 0){
		return -ENOENT;
	}
	inode = newfs_read_inode(dentry);

	if(inode->ftype == T_DIR){
		newfs_stat->st_mode = S_IFDIR | NEWFS_DEFAULT_PERM;
		newfs_stat->st_size = inode->dir_cnt * sizeof(struct newfs_dentry_d);
	}
	else if(inode->ftype == T_FILE){
	...
	newfs_stat->st_blksize = FS_BLOCK_SIZE;
	newfs_stat->st_blocks = MAX_OFF;
...
int newfs_readdir(const char * path, void * buf, fuse_fill_dir_t filler, off_t offset,
	...
	struct newfs_dentry *dentry, *sub_dentry;
	struct newfs_inode *inode;
	char pa[MAX_NAME_LEN];
	strcpy(pa, path);
	char name[MAX_NAME_LEN];
	...
	int cur_dir = offset;
	sub_dentry = get_son_dentry(inode, cur_dir);
	if(sub_dentry){
...
int main(int argc, char **argv)
{
	...
	newfs_options.device = strdup("/home/students/200210231/ddriver");
